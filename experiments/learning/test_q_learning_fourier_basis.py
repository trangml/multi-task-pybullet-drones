...
def main():
    ...
    for alpha in alphas:
        ...
        q_function = FourierBasis(num_obs, lims, order=order)

        weights = np.zeros((num_actions, q_function.numTerms))
        for episode in range(num_episodes):
            ...
            while not done:
                ...
                td_error = reward + gamma * (pred) - np.dot(weights, q_function.phi(state))[action]
