"""Updated for Multi-agent generalized tasking
...
To run the script, type in a terminal:
    ...
    $ python rl_multiagent.py --num_drones <num_drones> --env <env> --obs <ObservationType> --act <ActionType> --algo <alg> --num_workers <num_workers>
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        choices=["leaderfollower", "flock", "meetup", "multicrossobs"],
    ...
    elif ARGS.env == "multicrossobs":
        register_env(
            temp_env_name,
            lambda _: MultiCrossObstaclesAviary(
                num_drones=ARGS.num_drones,
                aggregate_phy_steps=shared_constants.AGGR_PHY_STEPS,
                obs=ARGS.obs,
                act=ARGS.act,
                ...
                # reward_components=[
                #     "EnterAreaReward": {"area": [[4, 5], [-1, 5]], "scale": 20}
                # ],
                # term_components=[{"OrientationTerm": None}],
            ),
        )
    else:
    ...
    if ARGS.env == "flock":
        ...
        temp_env = MultiCrossObstaclesAviary(
            num_drones=ARGS.num_drones,
            aggregate_phy_steps=shared_constants.AGGR_PHY_STEPS,
            obs=ARGS.obs,
            act=ARGS.act,
    ...
    config["multiagent"] = {
        ...
        "policy_mapping_fn": lambda x: "pol0"  # TODO: this is deprecated, Use `policy_mapping_fn(agent_id, episode, worker, **kwargs)`
        if x == 0
