...
class NavigateMazeAviary(BaseSingleAgentAviary):
    """Single agent RL problem: navigate through a maze."""
    ...
    def _addObstacles(self):
        ...
        super()._addObstacles()
        p.loadURDF(
            os.path.dirname(os.path.abspath(__file__)) + "/../../assets/maze.urdf",
            [1.5, -1.5, 0],
    ...
    def _computeReward(self):
        ...
        if np.allclose(np.asarray(state), np.asarray(target_dest), atol=0.1):
            ...
            return -1
        # ELSE if we hit a wall, reward is -100
        # ELSE give a reward of -1, to tell the drone to get moving
    ...
    def _clipAndNormalizeState(self, state):
        ...
        if self.GUI:
            self._clipAndNormalizeStateWarning(
                state, clipped_pos_xy, clipped_pos_z, clipped_rp, clipped_vel_xy, clipped_vel_z
            )
